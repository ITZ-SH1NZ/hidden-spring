...
export default function EndingScreen() {
  const { lives, completionTime } = useGame();
  ...
  const handleShare = () => {
    const timeStr = completionTime != null ? `${Math.floor(completionTime / 1000)}s` : "?s";
    const text = `I beat Hidden Spring in ${timeStr}! 🐣 Play at hiddenspring.game`;
    navigator.clipboard.writeText(text).catch(() => {});
  };

  const timeDisplay = completionTime != null ? `${Math.floor(completionTime / 1000)}s` : null;
        ...
        >
          ...
          {timeDisplay && (
            <div className="flex justify-between border-b border-white/10 pb-2">
              ...
              <span className="text-easter-yellow font-black">{timeDisplay}</span>
            </div>
          )}
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            ...
            {timeDisplay && (
              <button
                onClick={handleShare}
                className="px-8 py-4 bg-transparent border-2 border-easter-hotpink text-easter-hotpink font-black tracking-widest text-lg hover:bg-easter-hotpink hover:text-black transition-colors brutal-shadow"
              >
                SHARE YOUR TIME
              </button>
            )}
          ...
          {/* Credits */}
          <div className="border border-white/10 p-4 text-center text-white/30 text-xs tracking-[0.15em] leading-loose max-w-sm mx-auto">
            <div className="text-white/50 mb-1">HIDDEN SPRING — EASTER 2026</div>
            <div>Built with Next.js, Framer Motion, Web Audio API</div>
            <div>No images were harmed in the making of this game.</div>
          </div>
        </motion.div>
